...
const Preview: React.FC<PreviewProps> = ({ data, innerRef }) => {
  return (
    ...
    >
      ...
      {/* Main Content Grid */}
      <div className="grid grid-cols-12 gap-6 md:gap-12 items-start flex-grow z-10">
        {/* Left Column: Image, Name, Motivation, Signature */}
        <div className="col-span-6 flex flex-col space-y-5 md:space-y-8">
           {/* Profile Image */}
           ...
           {/* Name Banner */}
           <div className="bg-white py-4 md:py-6 px-3 md:px-5 rounded-2xl md:rounded-3xl text-center shadow-xl border-b-[4px] md:border-b-[8px] border-slate-200">
           ...
           <div className="space-y-6 md:space-y-10">
             <div className="space-y-3 md:space-y-5 bg-white/5 p-5 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-white/5 shadow-inner">
               <div className="text-green-400 text-[8px] md:text-[11px] font-black uppercase tracking-[0.2em] border-l-4 border-green-500 pl-3">
                  {data.motivationTitle}
               </div>
               <p className="text-[10px] md:text-[16px] font-medium leading-relaxed italic opacity-90 text-gray-200 line-clamp-5">
             ...
             <div className="w-full flex flex-col items-center pt-2">
                <div className="w-full flex items-center gap-4 mb-2">
                   ...
                   <div className="text-[7px] md:text-[10px] font-black text-gray-500 tracking-[0.4em] uppercase whitespace-nowrap">Authenticated By</div>
                   <div className="h-px bg-white/10 flex-grow"></div>
                </div>
        ...
        {/* Right Column: Attribute Rows */}
        <div className="col-span-6 flex flex-col space-y-3 md:space-y-5 pt-2">
      ...
      <div className="mt-auto pt-10 flex flex-col items-center z-10">
        <div className="flex flex-wrap justify-center gap-6 md:gap-12 text-[10px] md:text-[14px] font-bold tracking-[0.2em] text-gray-500 uppercase">
...
const AttributeRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="flex items-stretch min-h-[40px] md:min-h-[64px] bg-white/5 rounded-lg md:rounded-3xl overflow-hidden border border-white/10 group shadow-lg">
    <div className="bg-green-600 text-white flex items-center justify-center text-[7px] md:text-[11px] font-black w-[44px] md:w-[96px] px-1 text-center uppercase leading-none shrink-0 tracking-widest">
      {label}
    </div>
    <div className="flex-grow flex items-center px-3 md:px-6 py-2">
      <span className="text-gray-100 text-[9px] md:text-[15px] font-bold leading-snug line-clamp-3">{value || '---'}</span>
    </div>
  </div>
);
